...
import { Mic, Sparkles, Send, Bot, Star, User, ArrowLeft, MapPin, Square, TrendingUp, Bed, Bath, Car, ShieldCheck, Wifi, Trees, Calculator, FileText, Wallet, Coffee, Zap, Info, BarChart3, Loader2, ArrowRight, Heart, Map as MapIcon, Image as ImageIcon, ChevronLeft, ChevronRight, X, Building } from "lucide-react";
...
interface Property { id: string; title: string; price: number; priceFormatted: string; size: string; bedrooms: number; bathrooms: number; location: string;
  imageUrl: string; imageUrls?: string[]; purpose: 'buy' | 'rent'; isFeatured: boolean; category: string; subCategory: string; city: string; description?: string; paymentMode: string; installmentPlan?: string; agencyName: string; }
...
export default function PropertyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 🚀 THE FIX: Explicitly map safe properties to prevent 2.39M token crash
    ...
    try {
      const res = await fetch("/api/gemini", {
        ...
        body: JSON.stringify({
          history: newMessages,
          sessionId: sessionId, 
          currentProperty: safeCurrentProperty, // Only sending safe text data
            ...
            <div className="space-y-8 pb-8">
              <div>
                ...
                <p className="text-gray-300 leading-relaxed whitespace-pre-wrap text-lg opacity-90">
                  {property.description || `An exclusive ${property.size} ${property.subCategory} located in the highly sought-after ${property.location}. This premium listing is currently available on a ${property.paymentMode.toLowerCase()} basis.`}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-glass-gradient border border-white/10 p-6 rounded-3xl">
                  ...
                  <ul className="space-y-3">
                    ...
                    {property.isFurnished && <li className="flex items-center gap-3 text-gray-200"><CheckCircle2 className="w-5 h-5 text-emerald-400" /> Fully Furnished & Styled</li>}
